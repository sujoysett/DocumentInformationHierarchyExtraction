...
public class HTML2TJsonConvertor {
	...
	public static void analyzeHtml2() throws Exception {
			...
			if (style.contains("font-size")) {
				...
				if (fontSizeNum > headerToTextFontSizeBarrier) {
					...
					if (productDescriptionStarted) {
						...
						writeProcToIndividualFile(productDocs.size()+1, productDescriptionBody);
			...
			if (style.contains("font-size")) {
				...
				if (fontSizeNum > headerToTextFontSizeBarrier) {
					...
					if (productDescriptionStarted) {
						...
						String specificImmediateHeader = (context.get("25px")==null?"":context.get("25px")).toString();
						writeProcToIndividualFile(productDocs.size()+1, "**" + specificImmediateHeader +"**"+ "\n\n" +productDescription);
	...
	private static void writeProcToIndividualFile(int i, String productDescription) {
		BufferedWriter writer = null;
		try {
			FileOutputStream fileStream = new FileOutputStream("ProductUserManual/ProductUserManualStep"+ i+".txt");
